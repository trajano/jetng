...
public class DefaultParserContext implements ParserContext {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7992421081948651371L;
	/**
	...
	public void pushFile(final File file) throws IOException {
		if (file == NullFile.get() && !fileStack.isEmpty()) {
			throw new ParseException("Cannot pass a null file handle unless the parse context is empty", this);
		}
	...
	public void pushFile(final String fileName) throws IOException {
		final File includedFile = new File(currentFile.getParentFile(), fileName);
